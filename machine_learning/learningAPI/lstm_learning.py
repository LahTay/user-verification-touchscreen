...
class SaveLastBestModel(Callback):
    ...
    def on_train_end(self, logs=None):
        if self.best_weights is not None:
            ...
            formatted_save_path = self.base_save_path.format(epoch=self.best_epoch + 1, val_loss=self.best)
            self.model.save(formatted_save_path)
            if self.verbose > 0:
                print(f'The last best model weights were saved to {formatted_save_path}')
...
def load_data(path):
    ...
    time_data = tf.keras.preprocessing.sequence.pad_sequences(time_data, padding='post', value=2.0, dtype='float')
...
def split_data(data):
    ...
    for person, properties in data.items():
        for prop in ['true', 'false']:
            ...
            test.extend([(seq, 1 if prop == 'true' else 0) for seq in properties[prop][:n_test]])
            val.extend([(seq, 1 if prop == 'true' else 0) for seq in properties[prop][n_test:n_test + n_val]])
            train.extend([(seq, 1 if prop == 'true' else 0) for seq in properties[prop][n_test + n_val:]])
    ...
    new_data = Data(train_data, train_labels, test_data, test_labels, val_data, val_labels)
...
def augmentation(data):
    ...
    def calculate_change_metrics(array1, array2):
        ...
        cosine_sim = 1 - cosine(array1.flatten(), array2.flatten())  # 1 - cosine distance
...
def preprocessing(timedata, metadata):
    ...
    for t_data, meta in zip(timedata, metadata):
        ...
        data_by_signature_position[key].append((t_data, meta[0]))  # (position, Original|Fake)
...
def train(model: tf.keras.Model, data: Data, callbacks, optimizer, loss, metrics,epochs=100, batch_size=64):
    ...
    class_report = classification_report(data.y_test, predicted_labels, target_names=['Fake', 'Original'])
...
def build_lstm_model(input_shape):
    model = Sequential([
        Masking(mask_value=2., input_shape=input_shape),
...
if __name__ == '__main__':
    ...
    tensorboard_path = os.path.join("tensorboard_lstm", datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    history = train(model, preprocessed_data, [save_best_model, tensorboard_callback, early_stopping], 'adam',
                    'binary_crossentropy',['accuracy', Precision(), AUC(curve='PR')], 300, 64)
