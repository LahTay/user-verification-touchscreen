...
def download_and_rename_files(drive, folder_id, local_path, processed_files, processed_files_path, name_to_number_path):
    ...
    for file in file_list:
        ...
        if file['mimeType'] == 'application/vnd.google-apps.folder':
            download_and_rename_files(drive, file['id'], os.path.join(local_path, file['title']),
                            processed_files, processed_files_path, name_to_number_path)
        else:
            ...
            if file['title'] not in processed_files:
                ...
                if match:  # Already correct filename
                    ...
                    if number not in used_numbers:
                        ...
                        number = generate_unique_number(match.group(0), name_to_number, used_numbers)

                else:
                    name_match = re.match(r"([^\W_]+)(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s(\w+)\s(\d+)", file['title'])
                    if name_match:
                        ...
                        number = generate_unique_number(name_day_month_daynum, name_to_number, used_numbers)

                    else:
                        print(f"Error: Filename {file['title']} does not match expected patterns.")
                        continue
...
if __name__ == '__main__':
    ...
    used_numbers = set()
    download_and_rename_files(drive, main_folder_id, local_main_folder_path, processed_files, processed_files_path, name_to_number_path)
